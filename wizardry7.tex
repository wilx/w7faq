...
\makeatletter
\DeclareRobustCommand{\indexSpell}[1]{\index{#1}}
\DeclareRobustCommand{\spell}[1]{#1\indexSpell{#1}}
\DeclareRobustCommand{\indexPlace}[1]{\index{#1}}
\DeclareRobustCommand{\place}[1]{#1\indexPlace{#1}}
\DeclareRobustCommand{\indexRace}[1]{\index{#1}}
\DeclareRobustCommand{\race}[1]{#1\indexRace{#1}}
\DeclareRobustCommand{\indexClass}[1]{\index{#1}}
\DeclareRobustCommand{\class}[1]{#1\indexClass{#1}}
\DeclareRobustCommand{\indexMonster}[1]{%
  \IfEndWith{#1}{s}{\StrGobbleRight{#1}{1}[\WviiResult]%
  \index{\WviiResult}}{%
  \index{#1}}}
\DeclareRobustCommand{\monster}[1]{#1\indexMonster{#1}}
\DeclareRobustCommand{\monsterB}[2]{#1\indexMonster{#2}}
\DeclareRobustCommand{\indexNpc}[1]{\index{#1}}
...
\place{Starter Dungeon} or the one at Father Rulae), the \monster{Ra-sep-re-tep} pool
...
don't want to mistake \monster{Night Rooks} for \monster{Vampire Vultures}. In most cases
...
blasts, but not little laser beams like the ones the \monster{T'Rang Tecniks} use to
...
\monster{Myxlmynx} at level 7 (which are pretty darn useful). The best out of the three
summoning spells if you want magical damage.
...
\monster{H'Jenn-Ra}, meet Shritis (who you can say no to if you like the Umpani more,
...
Troopers and \monster{Savant Guards}.
...
return the Holy Work to \npc{Xen Xheng} as soon as possible and go looking for Xen
Xheng after he leaves the dojo. (Locate Person scrolls \textlcsc{will} help).
Ask \npc{Xen Xheng} about ``5 Flowers'' and see what he says. Try to get the
...
See, you're going to be fighting \monster{Frothing Munks} and a Leper Giant (two
...
thing about the \monster{Frothing Munks} though is that they throw up on you---and
you get diseased!
...
dangerous, but so is 6 \monster{Dark Forest Munks} that get a chance to cast stuff on
...
it. It closes that pit that dropped you into the \monster{Frothing Munks} and Leper
...
(where the fireballs are coming from) is the Temple of the \monster{Magna Dane}, which
you will be able to get to after passing the Temple of Wanderers and paying
for your membership as a Lord of Dane. Be warned that going down this
corridor means you will be taking heavy unpreventable (except by luck and
your fire resistances) fire damage and get into a fight with \monster{High
  Fathers} and other Danes after the gate at the very end. The fire damage
goes away after you kill \WviiSPAWN{} or talk (and take the Pit challenge)
with the \monster{Magna Dane} though, as the \monster{Magna Dane} will have left his Temple. I've
gotten stuck here before though, as the Pit exits here after a bit of winding
and my party just can't take 80--100+ HP fireballs at this point.
...
Talk to \monster{Magna Dane} to find out. You get to him by taking the stairs down
after going to the top of the tower, not by yanking a lever.

\faqentry{\monster{Magna Dane} is \textlcsc{hard}. Every time I start the fight, half my
...
Try it from the \monster{Magna Dane}'s chamber. You \textlcsc{did} see that secret button
right?
...
General Yamo, you'll be able to go into the \place{Spaceport Authority}
without problems.
...
It guards the \qmwordqm{BOAT} Map. Talk to \monster{H'Jenn-Ra} T'Rang in
...
The \place{Witch Mountains}. Have fun looking around. Watch out for \monster{Q'ua-tari}.
...
\faqentry{Where's \monster{H'Jenn-Ra}?}
...
\faqentry{\monster{H'Jenn-Ra} is killing my party! How do I deal with him?}

I guess you had a Pineapple after all. I won't lie to you---if your party is
not faster than \monster{H'Jenn-Ra}'s party, you're screwed unless you're on
...
can flatten those silly demons quickly, but the \monster{Myxlmynx} might be able to
...
\paragraph{Eagle Eye} Rescue \npc{Jan-Ette} from the \monster{T'Rang
  Assassins}. Give the Helazoid Pennant she gives you to Dame Ke-Li at
...
walk into the room to fight Pirannha Sharks, which are tougher \monster{Red
  Pirannhas}.
...
Skeleton Lords.  Defeat them for the Hi-Kane-Do (U and L), a Kabuto, a
...
instead of the Control Card in the slot to summon \monster{Savant Guards} and
Troopers.
...
seem easy on Easy are royal pains on Expert. The \monster{Magna Dane} fight
is a good example.
